...
try {
    ...
    tok = c.accessToken; sub = c.subscriptionType;
} catch { out({ok: false, error: 'no-credentials'}); process.exit(0); }
if (!tok) { out({ok: false, error: 'no-token'}); process.exit(0); }
...
try {
    ...
    if (res.status === 401 || res.status === 403) {
        if (cache) serveCache(cache, 'auth-expired');
        out({ok: false, error: 'auth-expired'}); process.exit(0);
    }
    if (!res.ok) {
